...
public sealed class StandAttribute : MonoBehaviour
{
    ...
    public void StartDebuff(int effectType, float duration)
    {
        ...
        switch (effectType)
        {
            case 4: // Steal Defense
                Stats stats =  GetComponent<Stats>();
                if (stats.shieldHp > 0) stats.TakeDamage(stats.maxHp/4);
                break;
            case 3: // Tripped
                // GetComponentInChildren<Animator>().SetBool("Trip", true);
                break;
            case 2: // Stunning //Also no need to be stopped here since the walking animation will block it
